...
worksheet = gc.open_by_key(SPREADSHEET_KEY).worksheet('シート2') #共有設定したスプレッドシートのシート1を開く
...
def mercari():
    ...
    for d in range(0, i): #各商品の進捗状況を取得
        ...
        if prog.text == "受取評価待ち": #進捗状況が「受取評価待ち」の時に次に進む
            ...
            try:
                if not worksheet.find(itm_id.text) == "": #D列に同じIDがある場合
    ...
    for i in range(0, i): #受取確認待ちの情報をスプレッドシートに転記
        ...
        if "受取確認待ち" in prog.text:
            ...
            if not "取引に問題" in ele.text: #取引に問題があるとrowの数が変わるので除外
                ...
                try:
                    if not worksheet.find(order_id.text) == "": #D列に同じIDがある場合
